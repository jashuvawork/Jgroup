...
export function EventsMediaGallery({ primary, accent }: EventsMediaGalleryProps) {
  const sections = getGroupedGalleryMedia();
  const photos = getGalleryPhotos();
  const [activeVideo, setActiveVideo] = useState<
    (ReturnType<typeof getGroupedGalleryMedia>[0]["items"][0]) | null
  >(null);
  ...
  return (
    <section id="gallery" className="relative border-t border-white/[0.06] px-6 py-20 md:py-28">
      <div className="mx-auto max-w-7xl">
        ...
        {/* Video reels grouped by package type */}
        {sections.map((section, si) => (
          <div key={section.title} className={si === 0 ? "mt-14" : "mt-16"}>
            <h3 className="mb-6 text-center text-xs tracking-[0.3em] uppercase text-white/35">
              {section.title}
            </h3>
